...
│   React 19 + TS + Vite           │     │   Kotlin + Jetpack Compose       │
│                                   │     │                                   │
│  Splash → Voice Greeting →       │     │  Mirrors the same screen flow     │
│  Checking → Answer (Home) →      │     │  natively (MainActivity,         │
│  My Street / Alerts / Map /      │     │  ui/screens/*, ui/components/*)  │
│  Impact / Ask Rahbar             │     │                                   │
│                                   │     │  Retrofit + OkHttp client         │
│  Header · BottomNav ·            │     │  Native STT / TTS                │
│  Profile Drawer · Emergency      │     │  Fused Location (GPS)            │
│  Modal · Location Selector       │     │                                   │
└────────────────┬──────────────────┘     └────────────────┬─────────────────┘
                 │ fetch() → JSON                          │ Retrofit → JSON
                 └──────────────────────┬────────────────────┘
                                        ▼
                       ┌─────────────────────────────────┐
                       │         EXPRESS SERVER              │
                       │           (server.ts)               │
                       │                                      │
                       │  POST /api/predict  → multi-agent    │
                       │  POST /api/chat     → bilingual chat │
                    ...
                    │  (gemini-3.6-flash)│   │  Live weather for          │
                    │  Grounded with real │   │  Islamabad (33.68, 73.04)  │
                    │  IESCO feeder data  │   └─────────────────────────────┘
                    │  in the system      │
                    │  prompt              │
                    └──────────────────────┘

              Local device storage (browser localStorage / Android prefs)
                 → lightweight user memory: name, detected street,
                   recent questions & predictions
```
